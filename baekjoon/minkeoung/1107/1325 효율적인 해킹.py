...
# 입력 처리
n, m = map(int, input().split())  # n: 컴퓨터의 수, m: 관계의 수
graph = [[] for _ in range(n + 1)]  # 각 컴퓨터 간의 관계를 저장할 그래프 초기화

# 그래프 입력: b가 해킹되면 a도 해킹 가능하다는 관계를 저장
for _ in range(m):
...
def bfs(v):
    ...
    visited = [False] * (n + 1)  # 방문 여부를 확인하기 위한 리스트
...
# 가장 해킹할 수 있는 컴퓨터 수 찾기
max_hack = max(result)  # 해킹할 수 있는 컴퓨터의 최대 수

# 최대 해킹 가능한 컴퓨터 수와 같은 값을 가진 컴퓨터를 출력
